 ...
 */
abstract class BaseQuickAdapter<T, K extends BaseRecyclerHolder> extends RecyclerView.Adapter<K> {
    ...
    public BaseQuickAdapter(Context context, List<T> datas, int itemLayoutId) {
        this.mContext = context;
        this.mData = datas;
        this.mLayoutResId = itemLayoutId;
    }
...
//        }
    ...
    public K onCreateViewHolder(ViewGroup parent, int viewType) {
        K BaseRecyclerHolder = null;
        this.mContext = parent.getContext();
        this.mLayoutInflater = LayoutInflater.from(mContext);
        switch (viewType) {
            case LOADING_VIEW:
                BaseRecyclerHolder = getLoadingView(parent);
                break;
            case HEADER_VIEW:
                BaseRecyclerHolder = createBaseRecyclerHolder(mHeaderLayout);
                break;
            case EMPTY_VIEW:
                BaseRecyclerHolder = createBaseRecyclerHolder(mEmptyLayout);
                break;
            case FOOTER_VIEW:
                BaseRecyclerHolder = createBaseRecyclerHolder(mFooterLayout);
                break;
            default:
                BaseRecyclerHolder = onCreateDefViewHolder(parent, viewType);
                bindViewClickListener(BaseRecyclerHolder);
        }
        //BaseRecyclerHolder.setAdapter(this);
        return BaseRecyclerHolder;

    }

    protected abstract void bindViewClickListener(K baseRecyclerHolder);

    //private void abstract  bindViewClickListener(BaseRecyclerHolder);

    private K getLoadingView(ViewGroup parent) {
        View view = getItemView(mLoadMoreView.getLayoutId(), parent);
        K holder = createBaseRecyclerHolder(view);
...
//        }
        return createBaseRecyclerHolder(parent, layoutId);
    }

    protected K createBaseRecyclerHolder(ViewGroup parent, int layoutResId) {
        return createBaseRecyclerHolder(getItemView(layoutResId, parent));
    }

    /**
     * if you want to use subclass of BaseRecyclerHolder in the adapter,
    ...
    protected K createBaseRecyclerHolder(View view) {
        Class temp = getClass();
        Class z = null;
        ...
        // 泛型擦除会导致z为null
        if (z == null) {
            k = (K) new BaseRecyclerHolder(view);
        } else {
            k = createGenericKInstance(z, view);
        }
        return k != null ? k : (K) new BaseRecyclerHolder(view);
    }
    ...
    public View getViewByPosition(RecyclerView recyclerView, int position, @IdRes int viewId) {
        ...
        BaseRecyclerHolder viewHolder = (BaseRecyclerHolder) recyclerView.findViewHolderForLayoutPosition(position);
        if (viewHolder == null) {
            return null;
        }
