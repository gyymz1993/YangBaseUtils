...
/**
 * @作者 Yang
 * @创建时间 2018/12/27 14:30
 * <p>
 * * 参数与相应结果.
 * * <br/>
 * * 通过{@link #(String)}或{@link #add(String, String)}来添加不同的BaseUrl，并通过create方法创建Api实现。
 * * <br/><br/>
 * * <pre>
...
public class ApiFactory {
    ...
    /**
     * 设置OkHttp连接超时时间，默认30秒，请在{@link #(String)}或{@link #add(String, String)}之前设置
     *
     * @param seconds 秒
     */
...
//                                "charset=UTF-8").addHeader("Accept-Encoding", "gzip, deflate")
...
//        String newBody = JSONObject.toJSONString(baseData);
        //将真实数据返回给回调
        Response res = response.newBuilder().body(ResponseBody.create(null, oldBody)).build();
        //Response res = response.newBuilder().body(ResponseBody.create(response.body().contentType(), newBody)).build();
        L_.e("TAG newBody" + res.toString() + ":" + oldBody);
        return res;
