...
import com.utils.gyymz.bar.style.TitleBarLightStyle;
import com.utils.gyymz.bar.style.TitleBarNightStyle;
import com.utils.gyymz.bar.style.TitleBarTransparentStyle;
...
public class NavigationBarView extends FrameLayout
    ...
    private void initStyle(AttributeSet attrs) {
        ...
        final TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.NavigationBarView);

        final ITitleBarStyle style;
        // 样式设置
        switch (array.getInt(R.styleable.NavigationBarView_barStyle, 0)) {
            case 0x10:
            ...
            case 0x30:
                ...
                break;
            default:
                style = NavigationBarView.sDefaultStyle;
                break;
        }

        // 标题设置
        if (array.hasValue(R.styleable.NavigationBarView_leftTitle)) {
            setLeftTitle(array.getString(R.styleable.NavigationBarView_leftTitle));
        }

        if (array.hasValue(R.styleable.NavigationBarView_title)) {
            setTitle(array.getString(R.styleable.NavigationBarView_title));
        } else {
        ...
        // 图标设置
        if (array.hasValue(R.styleable.NavigationBarView_rightTitle)) {
            setRightTitle(array.getString(R.styleable.NavigationBarView_rightTitle));
        }

        if (array.hasValue(R.styleable.NavigationBarView_leftIcon)) {
            setLeftIcon(getContext().getResources().getDrawable(array.getResourceId(R.styleable.NavigationBarView_leftIcon, 0)));
        } else {
            if (array.getBoolean(R.styleable.NavigationBarView_backButton, style.getBackIcon() != null)) {
                // 显示返回图标
                setLeftIcon(style.getBackIcon());
            }
        }

        if (array.hasValue(R.styleable.NavigationBarView_rightIcon)) {
            setRightIcon(getContext().getResources().getDrawable(array.getResourceId(R.styleable.NavigationBarView_rightIcon, 0)));
        }

        // 文字颜色设置
        setLeftColor(array.getColor(R.styleable.NavigationBarView_leftColor, style.getLeftColor()));
        setTitleColor(array.getColor(R.styleable.NavigationBarView_titleColor, style.getTitleColor()));
        setRightColor(array.getColor(R.styleable.NavigationBarView_rightColor, style.getRightColor()));

        // 文字大小设置
        setLeftSize(TypedValue.COMPLEX_UNIT_PX, array.getDimensionPixelSize(R.styleable.NavigationBarView_leftSize, (int) style.getLeftSize()));
        setTitleSize(TypedValue.COMPLEX_UNIT_PX, array.getDimensionPixelSize(R.styleable.NavigationBarView_titleSize, (int) style.getTitleSize()));
        setRightSize(TypedValue.COMPLEX_UNIT_PX, array.getDimensionPixelSize(R.styleable.NavigationBarView_rightSize, (int) style.getRightSize()));

        // 背景设置
        if (array.hasValue(R.styleable.NavigationBarView_leftBackground)) {
            setLeftBackground(array.getDrawable(R.styleable.NavigationBarView_leftBackground));
        } else {
            setLeftBackground(style.getLeftBackground());
        }

        if (array.hasValue(R.styleable.NavigationBarView_rightBackground)) {
            setRightBackground(array.getDrawable(R.styleable.NavigationBarView_rightBackground));
        } else {
            setRightBackground(style.getRightBackground());
        }

        // 分割线设置
        if (array.hasValue(R.styleable.NavigationBarView_lineColor)) {
            setLineDrawable(array.getDrawable(R.styleable.NavigationBarView_lineColor));
        } else {
            setLineDrawable(style.getLineDrawable());
        }

        setLineVisible(array.getBoolean(R.styleable.NavigationBarView_lineVisible, style.isLineVisible()));
        setLineSize(array.getDimensionPixelSize(R.styleable.NavigationBarView_lineSize, style.getLineSize()));
...
//        }
    ...
    public void setLeftIvIcon(int iconId) {
        if (iconId > 0) {
            setLeftIvIcon(getContext().getResources().getDrawable(iconId));
        }
    }
    ...
    public void setLeftIvIcon(Drawable drawable) {
        mletfIv.setImageDrawable(drawable);
        post(this);
    }
    ...
    public static void initStyle(ITitleBarStyle style) {
        NavigationBarView.sDefaultStyle = style;
    }
}